...
VALUES(6, 'Lê Văn Mới', 'com6.default.tp@gmail.com', '0936 978 033', 'Nam Cao - Hòa Khánh - Đà Nẵng', '099ebea48ea9666a7da2177267983138', 3);
SELECT * FROM nhanvien;
SELECT * FROM chitiettinhtrangdonhang;
DELETE FROM NhanVien WHERE nhanvien.TenNV = 'Lê Văn Mới';
DELETE FROM nhanvien WHERE nhanvien.TenNV =  'Lê Na'; -- = -> equals
-- BEST PRACTICE 
...
SELECT * FROM ChucVu;
DELETE FROM ChucVu 
WHERE TenCV LIKE '%giao hàng%'; -- LIKE 'expression' --> equals, startsWith, endsWith, contains

ALTER TABLE NhanVien DROP FOREIGN KEY fk_NhanVien_ChucVu1;
...
SELECT * FROM DonHang; -- X218.12.20 || 17.12.20 || 05.01.2021
SELECT MaDH, 
	   DiaChiGiaoHang, 
       ThoiGianDatHang, 
       CAST(ThoiGianDatHang AS DATE) AS NgayDatHang,
       CAST(ThoiGianDatHang AS TIME) AS ThoiGianDatHang 
FROM DonHang;
...
-- 5. Cập nhật các mặt hàng thuộc loại hàng là 'Mũ' với giá bán là 199K
...
SET GiaBan = 199
WHERE MaMH IN (SELECT * FROM MatHangAo); -- (1, 2, 11, 12)

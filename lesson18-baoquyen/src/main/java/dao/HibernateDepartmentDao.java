...
public class HibernateDepartmentDao extends AbstractHibernateDao implements DepartmentDao {
	...
	private static String Q_GET_TOTAL_BUDGET = "SELECT dept.dept_name " + DepartmentDto.DEPT_NAME + ",\n"
			+ "GROUP_CONCAT(concat(pro.pro_name, ' ', pro.budget) SEPARATOR ', ') "+ DepartmentDto.PRO_DETAIL + ",\n"
	...
	private static String Q_GET_DEPT_HIGHEST_BUDGET_PROJECT = "SELECT dept.dept_name " + DepartmentDto.DEPT_NAME +",\n"
	...
	public List<DepartmentDto> getDeptTotalBudget(int year) {
		return openSession().createNativeQuery(Q_GET_TOTAL_BUDGET)
	...
	public DepartmentDto getDeptHighestBudget(int year) {
		return  (DepartmentDto) openSession().createNativeQuery(Q_GET_DEPT_HIGHEST_BUDGET_PROJECT)
		.setParameter("year", year, IntegerType.INSTANCE)
		.addScalar(DepartmentDto.DEPT_NAME, StringType.INSTANCE)
		.addScalar(DepartmentDto.PRO_DETAIL, StringType.INSTANCE)
		.addScalar(DepartmentDto.DEPT_TOTAL, DoubleType.INSTANCE)

...
-- + Sử dụng refresh-insert-data.sql
...
VALUES(6, 'Lê Văn Mới', 'com6.default.tp@gmail.com','0772190059',  'Hòa Khánh - Đà Nẵng', '099ebea48ea9666a7da2177267983138');
SELECT * FROM nhanvien;
SELECT * FROM chitiettinhtrangdonhang;
DELETE FROM nhanvien WHERE TenNV = 'Lê Na'; -- equals
-- BEST PRATICE
-- A. FK Option: Restrict > Cascade
-- ALTER TABLE TBNAME DROP FOREIGN KEY FK_NAME
-- ALTER TABLE TABLE_NAME ADD CONSTRAINT_NAME FOREIGN KEY ... ON DELETE CASCADE || RESTRICT
-- B. Disable FK checks
ALTER TABLE MatHang DROP FOREIGN KEY fk_MatHang_LoaiHang1;
ALTER TABLE MatHang ADD CONSTRAINT FK_MatHang_LoaiHang FOREIGN KEY (MaLH) REFERENCES LoaiHang(MaLH)
ON DELETE CASCADE;
...
SELECT * FROM ChucVu;
DELETE FROM ChucVu
WHERE TenCV LIKE '%giao hàng%'; -- LIKE 'espressions' --> equals, startWith, endsWith, contains
...
-- 'yyyy-mm-dd' --> DATE
-- '###'		--> VARCHAR
SELECT * FROM DonHang; -- 18.12.20, 17.12.20, 05.01.21
SELECT MaDH,
	   DiaChiGiaoHang,
       ThoiGianDatHang,
...
-- CONVERT MANUALLY
-- 'dd/mm/yyyy' --> 
SELECT STR_TO_DATE('14/04/2022', '%d/%m/%Y');
SELECT * FROM DonHang;
...
--    MaTT = 5, TrangThai = 'Đã giao', ThoiGian = Thời gian hiện tại
...
SELECT * FROM HoaDon;
-- Opt 1: JAVA passed value(890) to database --> SAVED
...
-- TODO
-- 5. Cập nhật tất cả các mặt hàng thuộc loại hàng là 'Mũ' thành giá bán là 199K
...
WITH MatHangAo AS (
    ...
    FROM MatHang
    WHERE MaLH = (
		SELECT MaLH FROM LoaiHang WHERE TenLH = 'Áo'
    )
)
